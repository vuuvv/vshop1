...
class CaptchaTextInput(MultiWidget):
    def __init__(self,attrs=None, **kwargs):
        ...
        for key in ('text_field', 'image','hidden_field'):
            if '%%(%s)s'%key not in self._args.get('output_format'):
                raise ImproperlyConfigured('All of %s must be present in your CAPTCHA_OUTPUT_FORMAT setting. Could not find %s' %(
                    ', '.join(['%%(%s)s'%k for k in ('text_field', 'image','hidden_field')]),
                    '%%(%s)s'%key
                ))
...
class CaptchaField(MultiValueField):
    ...
    def __init__(self, widget_kwargs={}, *args,**kwargs):
        fields = (
        ...
        #)
        widget_kwargs["output_format"] = kwargs.get('output_format',None) or settings.CAPTCHA_OUTPUT_FORMAT
